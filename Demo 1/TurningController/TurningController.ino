...
void setup() {
  ...
  md.init();
  motor1Speed = 300;
  motor2Speed = -340; //the right motor needs to be driving in the opposite direction of the other motor for there to be positive rotation and no translation
  // ---
...
void loop() {
  ...
  // ---
  //takes the difference of the velocity (right velocity should be negative)
  ...
  if(phi > phiAndPhudge - controllerThreshold){
    double error = motorDecayFactor * (phiAndPhudge - x); //calculate the amount of distance left and multiply it by the motorDecayFactor
